        ...
        <div class="w-full pb-5">
            <div class="relative w-full mx-auto mt-4 overflow-x-auto sm:rounded-lg animate-fade_in_up">
                    ...
                    <tbody>
                        @foreach ($exercises as $exercise)
                            <tr
                                ...
                                <td scope="row"
                                    class="px-4 py-4 font-medium text-gray-900 break-words dark:text-white">
                                    <h1 class="text-base">{{ $exercise->name }}</h1>
                                </td>
                                <td class="px-6 py-4 text-base">
                                    <h2 class="max-w-[15rem] truncate">{{ $exercise->description }}</h2>
                                </td>
                                <td class="px-4 py-4 text-lg text-center capitalize">
                                    <h2>{{ $exercise->muscle_group }}</h2>
                                </td>
                                <td class="px-4 py-4 text-base text-center">
                                    <h2>{{ $exercise->created_at }}</h2>
                                </td>
                                <td class="py-4 text-center">
                                    <form action="{{ route('exercises.edit', $exercise) }}" method="get">
                                        @csrf
                                        <button type="submit"
                                ...
                                <td class="py-4 text-center">
                                    <form action='{{ route('exercises.delete', $exercise) }}' method="POST">
                                        @csrf
                                        <button type="submit"
                                ...
                                <td class="py-4 text-center">
                                    <form action='{{ route('exercises.show', $exercise) }}' method="get">
                                        @csrf
                                        <button type="submit"
                ...
                <div class="mt-10 mb-5">
                    {{ $exercises->links('pagination.custom-pagination') }}
                </div>
            </div>
        </div>
